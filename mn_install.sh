...
case $key in
    ...
    -h|--help)
    cat << EOL
IDChain Masternode installer arguments:
...
# These should automatically find the latest version of IDChain
...
BOOTSTRAPURL=$(curl -s https://api.github.com/repos/interactive-decisions-chain/idc-core/releases/latest | grep bootstrap.dat.xz | grep browser_download_url | cut -d '"' -f 4)
...
echo "
 ...
 |                                                  |
 |   Welcome to the IDChain Masternode Installer!   |::
 |                                                  |::
 +------------------------------------------------+::
   ::::::::::::::::::::::::::::::::::::::::::::::::::
"
...
if [[ ("$ADVANCED" == "y" || "$ADVANCED" == "Y") ]]; then
  ...
  echo "" && echo 'Added user "idchain"' && echo ""
  sleep 1
  ...
  if [ -z "$KEY" ]; then
    read -erp "Masternode Private Key (e.g. 3tdfjLCUzGczZi342w8GHp434R9kNY33eFyM58KRymkB56G4324h # THE KEY YOU GENERATED EARLIER) : " KEY
  fi
  ...
  if [[ "$I2PREADY" == "y" && "$TOR" != "y" && "$TOR" != "Y" && "$I2P" != "y" && "$I2P" != "Y" ]]; then
    ...
    read -erp "Would you like to use idchaind via TOR? [y/N] : " TOR
  fi
...
EOL
  ...
  I2PB32KEY=$(cat "$USERHOME/.kovri/client/keys/idchain.dat.b32.txt")
fi

# Install IDChain daemon
wget "$TARBALLURL"
tar -xzvf "$TARBALLNAME" -C /usr/local/bin
rm "$TARBALLNAME"

# Create .idchain directory
mkdir "$USERHOME/.idchain"

# Install bootstrap file
if [[ ("$BOOTSTRAP" == "y" || "$BOOTSTRAP" == "Y" || "$BOOTSTRAP" == "") ]]; then
  echo "Installing bootstrap file..."
  wget "$BOOTSTRAPURL" && xz -cd $BOOTSTRAPARCHIVE > "$USERHOME/.idchain/bootstrap.dat" && rm $BOOTSTRAPARCHIVE
fi

# Create idchain.conf
touch "$USERHOME/.idchain/idchain.conf"
...
cat > "$USERHOME/.idchain/idchain.conf" << EOL
${INSTALLERUSED}
bind=127.0.0.1
daemon=1
dnsseed=0
...
addnode=frlqcj2s3n6fhwmiyrgj6zhtara3upfw5p6netyiasq667uoygra.b32.i2p
addnode=
bind=127.0.0.1
daemon=1
dnsseed=0
...
chmod 0600 "$USERHOME/.idchain/idchain.conf"
chown -R $USER:$USER "$USERHOME/.idchain"
